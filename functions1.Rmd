...
```{r}
sales_eval <- function(prev, cont){
  if (cont>prev) return ('This salesperson is successful.') else return ('This salesperson is not successful.')
}
...
```{r}
final_mark <- function(mark){
  return(round((mark[1]*0.6 + mark[2]*0.4), digits = 0))
}
...
assess_sleep <- function(q){
  if(q>5) return('You can live, I believe.') else{
    if(q>3 & q<5) return('Well. Maybe you will survive.')
    if (q<3) return('Alarm! Are you still alive?')
  }
}
...
outliers_ind <- function(x){
  return(which(x<((quantile(x, prob = 0.25)) - 1.5*(quantile(x, prob = 0.75)  - quantile(x, prob = 0.25)))|(x>((quantile(x, prob = 0.75))+1.5*(quantile(x, prob = 0.75)  - quantile(x, prob = 0.25))))))
}
...
get_info <- function(x){
  dimensions <-  dim(x)
  columns  <- x$names
  rows.na <- which(!complete.cases(x))
  final_list <- list(dim(x), dimensions, rows.na)
  return(final_list)
}
...
dante <- function(){
  circle <- readline(prompt = ("В каком кругу Ада по Данте находится Платон?"))
  if (circle == "1" | circle == "первый" | circle == "Лимб") return ("Верный ответ!") else return ('Неверно. Перечитайте Данте!')
}
```
...
```{r}
motivate <- function(){
  word <- readline(prompt = ('Enter your word(s): '))
  if (tolower(word) == 'курсач'){
    print(sprintf('Every night in my dreams, I see you, %s.', word))
    cat('Great! You backed the right horse!')
    } else {
      print(sprintf('Every night in my dreams, I see you, %s.', word))
      cat('В смысле не курсач?')
  }
}
```